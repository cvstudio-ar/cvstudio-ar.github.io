(() => {
  ...
  function content() {
    ...
    const catalog = `<section class="evaluation-template-section"><div class="evaluation-section-title"><div><h3>Biblioteca de evaluaciones sectoriales</h3><p>Cada perfil contiene 22 consignas propias de su sector y genera un intento equilibrado por competencias.</p></div><span>${templates.length} perfiles activos</span></div><div class="evaluation-catalog-tools"><input type="search" data-template-search placeholder="Buscar por sector, puesto o competencia…" aria-label="Buscar una evaluación"></div><div class="evaluation-template-grid">${templates.map(template => `<article class="evaluation-template-card" data-template-card data-search="${esc([template.sector,template.tipo,template.titulo,template.descripcion,...(template.competencias || [])].join(' ').toLowerCase())}"><div class="evaluation-template-top"><span>${esc(template.sector || template.tipo)}</span><b>${template.duracion_minutos} min</b></div><h3>${esc(template.titulo)}</h3><p>${esc(template.descripcion || 'Simulacro de preparación laboral.')}</p><small>${template.version_contenido === '4.0' ? 'Sectorial completo' : esc(template.nivel || 'Intermedio')} · banco ${Array.isArray(template.preguntas) ? template.preguntas.length : 0} · intento ${template.cantidad_preguntas || template.preguntas?.length || 0}</small><button class="button primary small" data-template-use="${template.id}">Generar evaluación</button></article>`).join('') || '<div class="empty-state"><strong>No hay plantillas disponibles</strong><span>Verificá los permisos o activá al menos una plantilla.</span></div>'}</div></section>`;

...
Analizá integralmente el perfil del cliente. Redactá un CV profesional claro, convincente y compatible con ATS para el puesto objetivo. Priorizá las experiencias más relevantes, mejorá la redacción sin inventar información y entregá: Sobre mí, experiencias laborales optimizadas, formación, cursos, licencias, habilidades técnicas, competencias profesionales, palabras clave ATS y cierre.`;
  ...
  async function modalSignatureRequest(){
    ...
    try{
      ...
      document.getElementById('createSignatureLink')?.addEventListener('click',async e=>{e.currentTarget.disabled=true;e.currentTarget.textContent='Generando…';try{const result=await bridge.createSignatureRequest({clientId,clientName:client.name,phone:client.phone||'',documentName:'Carta de presentación'}),url=result.signingUrl,number=normalizeWhatsApp(client.phone),message=`Hola ${client.name.split(' ')[0]}, ya podés firmar tu carta de presentación desde este enlace exclusivo de CVStudio:\n\n${url}\n\nLa firma será utilizada únicamente en tu documento y se eliminará automáticamente 30 minutos después de enviarla.`;document.getElementById('signatureLinkResult').innerHTML=`<div class="signature-link-box"><strong>Enlace exclusivo generado</strong><input id="signatureGeneratedUrl" value="${esc(url)}" readonly><div><button class="button secondary small" id="copySignatureUrl">Copiar enlace</button>${number?`<a class="button primary small" href="https://wa.me/${number}?text=${encodeURIComponent(message)}" target="_blank" rel="noopener">Enviar por WhatsApp</a>`:''}</div></div>`;document.getElementById('copySignatureUrl').onclick=()=>navigator.clipboard.writeText(url).then(()=>toast('Enlace copiado.'));addActivity('client','Firma solicitada',`${client.name} · Carta de presentación`,client.id);saveState();}catch(err){toast(err.message);e.currentTarget.disabled=false;e.currentTarget.textContent='Solicitar firma';}});

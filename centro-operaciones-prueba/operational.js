...
Analizá integralmente el perfil del cliente. Redactá un CV profesional claro, convincente y compatible con ATS para el puesto objetivo. Priorizá las experiencias más relevantes, mejorá la redacción sin inventar información y entregá: Sobre mí, experiencias laborales optimizadas, formación, cursos, licencias, habilidades técnicas, competencias profesionales, palabras clave ATS y cierre.`;
  ...
  function clientWorkspaceOperational() {
    ...
    <section class="grid action-grid client-actions-single-row" aria-label="Acciones del cliente"><button class="action-card" data-action="open-whatsapp"><span>${brand('whatsapp')}</span>WhatsApp</button><button class="action-card" data-action="send-form"><span>${icon('file')}</span>Formulario</button><button class="action-card" data-action="register-payment"><span>${icon('dollar')}</span>Pago</button><button class="action-card" data-action="change-status"><span>${icon('refresh')}</span>Estado</button><button class="action-card" data-action="deliver-work"><span>${icon('upload')}</span>Entregar</button><button class="action-card" data-action="copy-client-sheet"><span>${icon('file')}</span>Copiar ficha</button><button class="action-card" data-action="more-actions"><span>${icon('settings')}</span>Más</button></section>
  ...
  function adminRenderer() {
    ...
    return `<section class="grid kpi-grid">${kpi('dollar','Ingresos confirmados',money(revenue),'desde pagos registrados','#35d07f')}${kpi('wallet','Disponible empresa',money(allocations.company),`${r.company}% de ingresos`,'#3b82f6')}${kpi('briefcase','Trabajos entregados',state.jobs.filter(j=>j.stage==='Entregado').length,'ejecuciones registradas','#9b5de5')}${kpi('users','Clientes',state.clients.length,'registros actuales','#ff8a1f')}${kpi('chart','Caja publicidad',money(growthAvailable),'asignado menos gastos','#ffd23f')}</section><section class="grid admin-grid">
      ...
      <section class="panel span-2"><div class="panel-head"><div><h2>Ingresos y egresos</h2><p>${confirmedPayments().length} pagos confirmados · ${state.expenses.length} gastos</p></div><button class="button secondary small" data-action="register-expense">Registrar gasto</button></div>${lineSvg(['#35d07f','#ff5d73','#3b82f6'])}</section>
      ${panel('Movimientos recientes',`<table class="data-table"><tr><th>Tipo</th><th>Detalle</th><th>Monto</th></tr>${[...confirmedPayments().map(p=>({type:'Ingreso',detail:`${p.client} · ${p.service} · ${p.paymentMethod||'Mercado Pago'}`,amount:p.amount})),...state.expenses.map(e=>({type:'Egreso',detail:e.description,amount:-e.amount}))].slice(-6).reverse().map(m=>`<tr><td>${m.type}</td><td>${esc(m.detail)}</td><td class="${m.amount<0?'negative':'positive'}">${m.amount<0?'-':''}${money(Math.abs(m.amount))}</td></tr>`).join('')}</table><button class="button secondary" data-action="register-payment" style="margin-top:12px;width:100%">Registrar pago</button>`)}
      <section class="panel span-2"><div class="panel-head"><div><h2>Servicios y precios</h2><p>Catálogo vigente utilizado en pagos y operaciones.</p></div><button class="button primary" data-action="edit-prices">Editar precios</button></div><table class="data-table"><tr><th>Servicio</th><th>Precio</th><th>Estado</th></tr>${Object.entries(state.prices).slice(0,7).map(([name,value])=>`<tr><td>${esc(name)}</td><td>${money(value)}</td><td><span class="status" style="--c:#35d07f">Activo</span></td></tr>`).join('')}</table></section>
      ${panel('Colaboradores y comisiones',`<table class="data-table"><tr><th>Colaborador</th><th>Trabajos</th><th>Comisión</th></tr>${Object.entries(collaboratorRows).map(([name,row])=>`<tr><td>${esc(name)}</td><td>${row.jobs}</td><td>${money(row.commission)}</td></tr>`).join('') || '<tr><td colspan="3">Sin ejecuciones entregadas</td></tr>'}</table>`)}
    </section>`;
  }

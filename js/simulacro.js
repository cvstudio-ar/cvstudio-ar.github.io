(() => {
  ...
  function renderIntro() {
    app.innerHTML = `<div class="test-state"><div class="test-state-icon">✓</div><span class="question-label">Preparación personalizada</span><h1>${esc(evaluation.title)}</h1><p>Hola ${esc(evaluation.first_name)}. Leé las consignas con calma y elegí una sola respuesta por pregunta. El cronómetro comenzará únicamente cuando presiones el botón.</p><div class="test-meta"><div><small>Duración</small><strong>${evaluation.duration_minutes} minutos</strong></div><div><small>Preguntas</small><strong>${evaluation.questions.length}</strong></div><div><small>Modalidad</small><strong>Un intento</strong></div></div><button class="test-button" id="startTest">Comenzar simulacro</button><p class="test-legal">No cierres esta ventana durante la actividad. Tus respuestas se enviarán automáticamente al finalizar el tiempo.</p></div>`;
  ...
  function renderQuestion() {
    ...
    app.innerHTML = `<div class="test-top"><div><h1>${esc(evaluation.title)}</h1><p>Pregunta ${current + 1} de ${questions.length}</p></div><div class="test-timer" id="testTimer">${formatClock(remainingSeconds())}</div></div><div class="test-progress"><i style="width:${progress}%"></i></div><section class="question-card"><span class="question-label">${esc(question.category || 'Pregunta')}</span><h2>${esc(question.text)}</h2><div class="question-options">${question.options.map((option,index) => `<button class="question-option ${selected === option.value ? 'is-selected' : ''}" data-answer="${esc(option.value)}"><b>${String.fromCharCode(65+index)}</b><span>${esc(option.label)}</span></button>`).join('')}</div></section><div class="question-nav"><button class="test-button secondary" id="previousQuestion" ${current === 0 ? 'disabled' : ''}>Anterior</button><div class="question-dots">${questions.map((item,index) => `<button class="question-dot ${index === current ? 'is-active' : ''} ${answers[item.id] ? 'is-answered' : ''}" data-question="${index}" aria-label="Pregunta ${index+1}">${index+1}</button>`).join('')}</div>${current === questions.length - 1 ? '<button class="test-button" id="finishTest">Finalizar</button>' : '<button class="test-button" id="nextQuestion">Siguiente</button>'}</div>`;
